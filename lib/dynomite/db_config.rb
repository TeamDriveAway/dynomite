...
module Dynomite::DbConfig
  ...
  module ClassMethods
    ...
    def check_dynamodb_local!(endpoint)
      ...
      open = port_open?(uri.host, uri.port, 0.2)
      unless open
        raise "You have configured your app to use DynamoDB at endpoint, #{uri} , but it is not running.  Please start DynamoDB local. Example: brew cask install dynamodb-local && dynamodb-local"
      end
    end
